...
way you want. insert_field_before() and insert_field_after() are similar
...
=head2 Writing records to a file
  ...
  12      );
  13   $record->append_field($author,$title);
  14   
...
As its name suggests append_field() will add the 590 field to the end of
the record. If you want to preserve a particular order you can use the 
insert_field_before() and insert_field_after(). In order to use these you 
...
example:
  ...
  22       $record->insert_field_before($before,$new);
  23   
  24       print OUT $record->as_usmarc();
  25   
  26   }

insert_field_after() works in a simliar fashion to insert_field_before()
...
Perl's localtime() to generate the data we need for this field.
  ...
  23     $record->append_field('005',$datetime);
  24   
...
You may find yourself in the situtation where you would like to
...
Here is the example in detail:
  ...
  10   
  11     ## if the record doesn't contain an 856 field we can skip it 
  12     next if !$existing; 
  13   
  ...
  23     push(@subfields,'z','Access restricted','u',$existing->subfield('u'));
  24   
  25     ## create a new 856 field using the new reordered subfields
  26     my $new = MARC::Field->new( 
  27       '856', $existing->indicator(1), $existing->indicator(2), @subfields
  28       );
  29   
  30     ## replace the existing subfield with our new one
  31     $existing->replace_with($new);
  32   
  33     ## write out the new record
  34     print OUT $record->as_usmarc();
  35   
  36   }
...
yet. Feel free to suggest new ones. 
