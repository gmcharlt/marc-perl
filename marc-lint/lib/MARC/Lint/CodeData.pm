...
%Sources655 = map {($_, 1)} (split "\t", ("aat	aktp	amg	asth	barn	bgtchm	bt	cash	cct	conorsi	czenas	dacs	dcs	dct	ericd	estc	fast	fbg	fire	galestne	gem	gmgpc	gsafd	gst	gtlm	hapi	hkcan	hoidokki	ica	ilot	itglit	itrt	jhpb	jhpk	kkts	lacnaf	lcsh	lcshac	lctgm	lemac	local	maaq	mar	marcgt	mesh	migfg	mim	nal	nalnaf	ngl	nimafc	nlgaf	nlgkk	nlgsh	nlmnaf	nmc	nznb	opms	pmcsg	qrmak	qtglit	raam	radfg	rbbin	rbgenr	rbpap	rbpri	rbprov	rbpub	rbtyp	reo	rerovoc	reveal	rma	rugeo	rvm	sao	sears	sgc	sgce	sgp	sipri	snt	socio	swemesh	tbit	tho	thub	toit	tsht	ulan	vmj	waqaf"));
